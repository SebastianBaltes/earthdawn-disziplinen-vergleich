  ...
  function refreshResult() {
    ...
    _.each(disziplin2Schaden,function(d2s){
      var schadenEquals = function( x, y ) {
        var a = x.schaden;
        var b = y.schaden;
        return a.length == b.length && a.every( function(this_i,i) { return Math.abs(this_i - b[i])<2; } );  
      };
      var i=1;
...
//      var median = line[Math.round(line.length/2)];
      ...
      var geometrischesMittel = Math.pow(_.foldl(line, function(a,b){return a*b;}, 1), 1/line.length);
...
//        transform:  function(v) { return Math.pow(v,0.5); } 
        transform:  function(v) { return v<60.1?v:60+Math.pow(v-60,0.5); } 
      },
      legend: {
        show: true,
        position: "nw"
...
//                }
      ...
      $('.details table').addClass('fancyTable').fixedHeaderTable({ fixedColumns:4, autoShow: true, footer: false, height: 600, altClass: 'odd'});
