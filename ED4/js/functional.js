...
function funValues(funs,x) {
  return _.map(funs,function(f){ return funValue(f,x); });
}

function funValue(fun,x) {
  if (_.isNull(fun) || _.isUndefined(fun)) {
    return 0;
  }
  if (_.isFunction(fun)) {
...
function add() {
  var funs = arguments;
  return function(x) {
    return _.foldl(funValues(funs,x), function(memo, num){ return memo + num; }, 0);
  };
}
...
function mul() {
  var funs = arguments;
  return function(x) {
    return _.foldl(funValues(funs,x), function(memo, num){ return memo * num; }, 1);
  };
}
...
function sumOver(key) {
  return function(x) {
    ...
    return _.foldl(list, function(memo, num){ return memo + num; }, 0);
  };
}
